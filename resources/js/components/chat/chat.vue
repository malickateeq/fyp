<template>
    ...
    <h4 class="mt-2 text-center"> <i class="fa fa-envelope" aria-hidden="true"></i> Messaging </h4> 
    <hr>

        <div class="row">

            <div v-show="myAllMessages == '' " class="text-center">
                <h2> No message receive. </h2>
            </div>
...
<div class="col-md-8 col-md-offset-2 my-5">
    ...
    <div v-if="friendId!=''">
        <div class="card">
            ...
            <ul class="list-group list-group-flush chat-body" ref='msgBody' id="chatBackground" v-chat-scroll="{always: false, smooth: true}">
        ...
        methods:{
            ...
            updateStatuses(){
                for(let i=0;i<this.onlineUsers.length;i++)
                {
                    ...
                    if(this.friendId == this.onlineUsers[i].user_id)
                    {
                        this.online = 'yes';
            ...
            setTypingUser(){
                console.log('event');
                if(this.test.user_id==this.friendId)
                {
                    console.log('changed');
            ...
            initListeners(){
                if(this.lock == '0')
                {
                    console.log('Initializing listeners, Me: '+ this.myId);
            ...
            getUserPic(index){
                if(this.myAllMessages[index][0]['friend_image'] != null)
                {
                let pic = (this.myAllMessages[index][0]['friend_image'].length > 200) ? this.myAllMessages[index][0]['friend_image'] : this.baseURL+"/img/profile/"+ this.myAllMessages[index][0]['friend_image'];
                return pic;
                }
                // else{
                // let pic = (this.myAllMessages[index][0]['friend_image'].length > 200) ? this.myAllMessages[index][0]['friend_image'] : "/img/profile/"+ this.myAllMessages[index][0]['friend_image'];
                // return pic;
                // }
            },
            uploadFile(e){
